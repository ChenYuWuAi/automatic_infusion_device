...
std::string rpc_powerState_fn(const json &params)
{
    ...
    {
        // 设置泵状态为输液状态
        if (g_stateMachine->getState() == VERIFIED)
        {
            g_stateMachine->setState(PREPARING);
            InfusionLogger::info("泵状态设置为准备状态，目标流量: {}ml/h", g_pumpParams.target_flow_rate.load());
        }
        else if (g_stateMachine->getState() == PAUSED)
        {
            g_stateMachine->setState(INFUSING);
            InfusionLogger::info("泵状态设置为输液状态，目标流量: {}ml/h", g_pumpParams.target_flow_rate.load());
        }
        else if (g_stateMachine->getState() == IDLE)
        {
            g_stateMachine->setState(VERIFY_PENDING);
            InfusionLogger::info("泵状态设置为验证状态");
        }
        else
        {
            InfusionLogger::warn("泵当前状态不允许启动");
            json error_json;
            error_json["error"] = "Pump is not in a valid state to start";
            return error_json.dump();
        }
    }
    else if (params == 2)
    ...
    {
        // 设置泵状态为停止状态
        g_stateMachine->setState(IDLE);
        InfusionLogger::info("泵状态设置为停止状态");
    }
...
std::string rpc_getSystemDiagnostics_fn(const json &params)
{
    ...
    diagnostics["timestamp"] = static_cast<uint64_t>(std::chrono::system_clock::now().time_since_epoch().count() /
                                                     1000000);
