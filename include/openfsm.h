...
namespace openfsm {
    ...
    public:
        ...
        static bool
        RegisterState(const std::string &stateName, std::vector<std::string> &vectActionName, int eState = -1) {
            return OpenFSMPool_.registerState(stateName, vectActionName, eState);
        }

        static bool
        RegisterState(const std::string &stateName, const std::initializer_list<std::string> &list, int eState = -1) {
